...
from core.database.sync import push_chapter_to_cloud
...
class SummaryManager:
    ...
    def generate_chapter_summaries(self, use_local_ai=False, model_name=None):
        """Generates summaries using stateful context fetched from Google Drive."""
        todo = (
            self.db.query(ChapterProcessing)
            .join(Chapter)
        ...
        for count, proc in enumerate(todo, 1):
            ...
            # Fetch OCR text from Google Drive via drive_file_id
            if not chapter.drive_file_id:
                # Data integrity check: ocr_extracted=True but no Drive file reference
                error_msg = (
                    f"❌ CRITICAL: Ch {chapter.chapter_number} marked ocr_extracted=True "
                    f"but drive_file_id is missing. Data inconsistency detected."
                )
                logger.critical(error_msg)
                raise Exception(error_msg)

            try:
                ocr_text = self.drive_client.download_text(chapter.drive_file_id)
            except Exception as e:
                # Drive file missing, network error, auth failure, etc.
                error_msg = (
                    f"❌ CRITICAL: Failed to fetch OCR from Drive for Ch {chapter.chapter_number} "
                    f"(drive_file_id: {chapter.drive_file_id}): {e}"
                )
                logger.critical(error_msg)
                raise Exception(error_msg)
            ...
            try:
                ...
                if ai_results:
                    ...
                    proc.summary_complete = True
                    self.db.commit()
                    print(f"✅ Ch {chapter.chapter_number} → Summary saved")

                    # Cloud Sync Trigger
                    print(f"☁️ Syncing Ch {chapter.chapter_number} to Cloud Replica...")
                ...
                self.db.rollback()
                # Fail fast to maintain context integrity
                error_msg = f"❌ CRITICAL ERROR on Ch {chapter.chapter_number}: {e}"
                logger.critical(error_msg)
                print(error_msg)
            ...
            if not use_local_ai and count < len(todo):
                ...
                print(f"⏸️ Pacing {model_name} ({sleep_time}s)...")
